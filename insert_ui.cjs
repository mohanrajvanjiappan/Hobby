...
const searchPoint = `                {stage === 'reveal' && (
                  <motion.div
                    initial={{ scale: 0.9, opacity: 0 }}
                    animate={{ scale: 1, opacity: 1 }}`;
const insertUI = `                {quiz.mode === 'interactive' && stage !== 'reveal' && (
                  <motion.div
...
`;
fs.writeFileSync('src/components/Presentation.tsx', content.replace(searchPoint, insertUI + searchPoint));
